...
public class CRoom : MonoBehaviour
{
    ...
    [SerializeField]private int m_EnemyNum,m_BoxNum,m_Num;       //敌人、箱子数量和总数量
    [SerializeField]private float X1, Y1, W, H;                  //房间范围
    private GameObject Door,Takarabox;
    private readonly List<Vector3> m_RandomPos=new List<Vector3>();
    private readonly List<GameObject> m_Enemies=new List<GameObject>();
    ...
    private void Start()
    {
        ...
        Takarabox = GenerateFromAsset("takarabox", m_RandomPos[m_Num], CTool.s_ZeroQuaternion);
    }
    ...
    private void GenerateEnemies()
    {
        ...
        {
            m_Enemies.Add(GenerateFromAsset("box", m_RandomPos[i], CTool.s_ZeroQuaternion));
        }
        for (int i = m_BoxNum; i < m_Num; i++)
        {
            tempName = "enemy" + RandomID();    //怪物的对象名只有最后的数字不同
            m_Enemies.Add(GenerateFromAsset(tempName, m_RandomPos[i], CTool.s_ZeroQuaternion));
        }
    }
    private void ActivateEnemies()
    {
        ...
        for (int i = m_BoxNum; i < m_Num; i++)
        {
            ...
            m_Enemies[i].GetComponent<CEnemy>().ConnectToRoom(this);
        }
    }
    //离开房间
    private void GetOut()   
